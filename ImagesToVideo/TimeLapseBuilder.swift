...
public class TimeLapseBuilder: NSObject {
  ...
  func build2(progress: (NSProgress -> Void), success: (NSURL -> Void), failure: (NSError -> Void)) {
    ...
    let videoOutputURL = documentDirectory.URLByAppendingPathComponent("TempVideo.mov")
